...
def interproScan(subject_info, all_proteins,pfam_domains_file, email, developing, script_dir):
    '''Scans all the proteins in protein_list with InterproScan. Remove subjects from subject_info that have a domain that is not allowed (as given by user, e.g. transcription factor domains)
        or that don't have any IPR domains in common with any of its query proteins.
    ...
    for fasta_sequence in sorted(fasta_sequences):											 # loop over all the sequences
        ...
        if not os.path.isfile(interpro_file):
            ...
            if sequence_count % 25 == 0 or len(fasta_sequences) <= sequence_count-1:	  # divide it up into chunks of 25
                y = 0
                while True:
                    try:
...
#                    print 'not set(protein_ipr_db_domain[subject][\'ipr_domain_names\']).isdisjoint(protein_ipr_db_domain[query][\'ipr_domain_names\']):',not set(protein_ipr_db_domain[subject]['ipr_domain_names']).isdisjoint(protein_ipr_db_domain[query]['ipr_domain_names'])
                ...
                if len(protein_ipr_db_domain[subject]['ipr_domain_names']) == 0: # add proteins without domains
